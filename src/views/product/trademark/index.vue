<template>
  <div>
    <el-card class='box-card'>
      ...
      table:
        ...
        table_column:
          ...
          align=>列对齐方式
      -->
      <el-table ref='tableRef' class='tableClass' :data='trademarkArr'
                :height='tableHeight' :max-height='tableHeight' border stripe>
        <el-table-column label='序号' type='index' :index='(pageNo-1)*pageSize+1'
                         width='80px' align='center' fixed>
        </el-table-column>
        <el-table-column label='品牌名称' prop='tmName'>
        </el-table-column>
        <el-table-column label='品牌logo' prop='tmName'>
          <template v-slot='{row,$index}'>
            <img :src='row.logoUrl' style='width:50px;height:50px;' alt='图片找不到了' />
          </template>
        </el-table-column>
    ...
    <el-dialog v-model='dialogVisible' :title='operType'>
      <el-form ref='trademarkFormRef' :model='trademarkForm' :rules='trademarkRules'
               style='width:80%' label-width='120px'>
        ...
        <el-form-item label='品牌LOGO：' prop='logoUrl'>
          <el-upload ref='avatarUploader' class='avatar-uploader' :action='PICTURE_UPLOAD_URL' :show-file-list='true'
      ...
      <template v-slot:footer>
        ...
        <el-button type='primary' size='default' @click='confirm' :loading='loading'>确定</el-button>
      </template>
    </el-dialog>
  </div>
</template>
...
import type { FormInstance, FormRules, UploadFile, UploadInstance, UploadProps, UploadRawFile } from 'element-plus'
import { ElMessage } from 'element-plus'
import { PICTURE_UPLOAD_URL, reqAddOrUpdateTrademark, reqTrademark } from '@/api/product/trademark'
...
const { tableRef, tableHeight, pageNo, pageSize, total, handleSizeChange, handleCurrentChange }
...
// 定义收集新增品牌的数据
const trademarkForm = reactive<Trademark>({ id: undefined, tmName: '', logoUrl: '' })
// 定义表单校验需要的配置对象
const trademarkRules = reactive<FormRules<Trademark>>({
   ...
   */
  tmName: [
    { required: true, min: 2, message: '品牌名称长度至少2位', trigger: 'change' },
  ],
  logoUrl: [
...
const confirm = async () => {
  try {
    ...
    loading.value = false
    ElMessage.error({ dangerouslyUseHTMLString: true, message: (error as Error).message })
  }
}
</script>
